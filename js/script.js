$(document).ready(function(){
  ...
  var config = {
    ...
    projectId: "smarkt-bac7b",
    storageBucket: "smarkt-bac7b.appspot.com",
    messagingSenderId: "205611556385"
  };
  ...
  var salesRef = dbRef.ref('sales')
  var contactsRef = dbRef.ref('contacts')
  var usersRef = dbRef.ref('users')
  var trolleyRef = dbRef.ref('trolley')
  var auth = null;
  ...
  $('#doLogin').on('click', function (e) {
    ...
    if( $('#loginEmail').val() != '' && $('#loginPassword').val() != '' ){
      ...
      var newObj = {time: Date.now() }; 
      var timelog = {timelogin: Date.now()};
      var trolley_num = {user: data.email};
      var log = data.email
      firebase.auth().signInWithEmailAndPassword(data.email, data.password)
        .then(function(authData) {
          ...
          auth = authData;
          usersRef.child(auth.uid).child('logins').push(newObj);
          salesRef.child('foo').child('lista').set(timelog);
          trolleyRef.child('1').set(trolley_num);
          $('#messageModalLabel').html(spanText('Success!', ['center', 'success']))
          setTimeout(function () {
            $('#messageModal').modal('hide');
            $('.unauthenticated, .userAuth').toggleClass('unauthenticated').toggleClass('authenticated');
